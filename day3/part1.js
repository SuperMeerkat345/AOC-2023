...
tempInput = input.trim().split("\n");

// (x, y)
function getSymbolCoords () {
    symbolCoords = []; // reset

    for (let rowNum in tempInput) {
        let row = tempInput[rowNum];

        for (let charNum in row) {
            let char = row[charNum]
            
            if (symbols.includes(char)) { // is the character a symbol
                // (x, y) format
                let tempCoord = [parseInt(charNum), parseInt(rowNum)]

                symbolCoords.push(tempCoord)
            }
        }
    }
}
...
// (x, y, number) where (x, y) is the starting char
function getNumberCoords () {
    for (let rowNum in tempInput) {
        let row = tempInput[rowNum];
        let tempCoord = [];
        ...
        for (let charNum in row) {
            ...
            if (isNumber(char)) { // a number
                if (tempCoord.length === 0) { // new num
                    tempCoord[0] = parseInt(charNum); // x
                    tempCoord[1] = parseInt(rowNum); // y
                    tempCoord[2] = char; // number
                }
                else { // number already exists, so keep adding to it
                    tempCoord[2] += char; // add the next digit to the number
                }
            }
            else { // not a number
                if (tempCoord.length !== 0) { // send collected number to coords if there is a collected number
                    tempCoord[2] = parseInt(tempCoord[2]); // make the number an actual number
                    numberCoords.push(tempCoord); // send the number to the coords list
                }
                tempCoord = []; // reset the temp coord for next number
            }
        }

        // Check if there is still a number to push after the row ends
...
// gets valid numbers and puts them into the valid nums array
function getValidNumbers() {
    for (let numberCoord of numberCoords) {
        let numberX = numberCoord[0]; // x
        let numberY = numberCoord[1]; // y
        let numberLength = String(numberCoord[2]).length; // length of the number

        let numberPushed = false; // flag for if the number has been verified

        // Loop through all positions of the number (handle multi-digit numbers)
        for (let i = 0; i < numberLength; i++) {
            let currentX = numberX + i; // Current x position for this part of the number

            for (let symbolCoord of symbolCoords) {
                let symbolX = symbolCoord[0]; // x
                let symbolY = symbolCoord[1]; // y

                // Check all surrounding positions
                if (
                    (symbolY === numberY - 1 && (symbolX === currentX - 1 || symbolX === currentX || symbolX === currentX + 1)) || // Top row
                    (symbolY === numberY && (symbolX === currentX - 1 || symbolX === currentX + 1)) || // Middle row, left and right
                    (symbolY === numberY + 1 && (symbolX === currentX - 1 || symbolX === currentX || symbolX === currentX + 1)) // Bottom row
                ) {
                    validNums.push(numberCoord[2]); // Add the entire number
                    numberPushed = true;
                    break; // No need to check further if this number is valid
                }
            }

            if (numberPushed) break; // Stop if the number has been pushed
        }
    }
}
...
for (let coord of numberCoords) {
    //console.log("Number Coordinates:", coord);
}

//console.log(numberCoords[numberCoords.length - 1]);

//console.log("Numbers Coordinates:", numberCoords);
//console.log("Valid Numbers", validNums);
//console.log("temp:", temp);

console.log("Answer:", answer); //returns 529618