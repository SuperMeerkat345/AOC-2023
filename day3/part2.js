...
function getGearCoords () {
    ...
    for (let rowNum in tempInput) {
        ...
        for (let charNum in row) {
            ...
            if (char === "*") { // is the character a *
                // (x, y) format
                let tempCoord = [parseInt(charNum), parseInt(rowNum)]

                gearCoords.push(tempCoord)
            }
...
function getValidGearRatios () {
    for (let gearCoord of gearCoords) { //console.log(1);
        let gearX = gearCoord[0]; // X
        let gearY = gearCoord[1]; // Y

        let gearNumbers = [] // numbers around the gear


        for (let numberCoord of numberCoords) { //console.log(2);
            let numberX = numberCoord[0] // X
            let numberY = numberCoord[1] // Y
            let numberLength = String(numberCoord[2]).length;
            

            let numberPushed = false; // flag for if the number has been verified

            for (let i = 0; i < numberLength; i++) { 
                let currentX = numberX + i;

                let leftCol = gearX - 1 === currentX;
                let middleCol = gearX === currentX;
                let rightCol = gearX + 1 === currentX;

                let topRow = gearY - 1 === numberY;
                let middleRow = gearY === numberY;
                let bottomRow = gearY + 1 === numberY;

                if (
                    (leftCol && topRow || middleCol && topRow || rightCol && topRow) ||
                    (leftCol && middleRow || middleCol && middleRow || rightCol && middleRow) ||
                    (leftCol && bottomRow || middleCol && bottomRow || rightCol && bottomRow)
                ) {
                    numberPushed = true;
                    gearNumbers.push(numberCoord[2]);
                    break;
                }
        ...
        if (gearNumbers.length === 2) {
            let tempVal = gearNumbers[0] * gearNumbers[1];
            validGearRatios.push(tempVal);
            console.log(tempVal);
        }
    }
}
...
function getAnswer () {
    ...
    for (let val of validGearRatios) {
        answer += val;
    }
}
...
//console.log("Possible gear coords: ", gearCoords);
//console.log("Last gear coord: ", gearCoords[gearCoords.length - 1]);

console.log("Number coords: ", numberCoords);
//console.log("Last number coord: ", numberCoords[numberCoords.length - 1]);

console.log(validGearRatios);
