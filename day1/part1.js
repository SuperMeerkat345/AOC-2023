...
function getFirstNum (row) {
    for (let char of row) {
        if (is_numeric(char)) {
            return char; //return the number as a string so we can combine with last number
        }
    }
}

function getLastNum (row) {
    for (let char = row.length; char > -1; char--) {
        if (is_numeric(row[char])) {
            return row[char]; //return the number as a string so we can combine with last number
        }
    }
}
...
function getAllRows () {
    for (let row of input) {
        ...
        let firstNum = getFirstNum(row)
        let lastNum = getLastNum(row)
...
console.log(answer); //returns 54561